...
| plugin.json `categoryId` **原串**（#23；插件级，团队全员共享；无则卡片 undefined——实测 4/42 插件缺失） | `category`（筛选键：客户端分类 chip、`workbuddy_experts` 的 `category` 参数；标签翻译只在展示边缘发生，见 #23） |
...
- **丢弃字段**（DSH 无对应物，如实记录；#18 补全）：agent frontmatter 的 `maxTurns`、`agentMode`、`enabled*`、`vibe`/`emoji`/`color`（3 文件有）；plugin.json 的 `version`、`homepage`、`defaultInitPrompt`、`quickPrompts`、`tags`（zh/en 标签——`categoryId` 曾同列，#23 复活为 `category` 字段，tags 维持丢弃）；SKILL.md 的 `allowed-tools` 门控。
...
| 23 | 分类筛选（#12 拷问轮砍掉项的复活，实施新增） | **采纳 plugin.json `categoryId` 为卡片 `category` 字段**，推翻 #12/#18 中「丢弃 categoryId」的记录（tags 维持丢弃）：**① 原串透传**——`"02-Engineering"` 等 WorkBuddy 市场分组键逐字上卡（插件级，团队全员共享；实测 42 插件中 4 个无此字段 → 卡片 undefined，不伪造「未分类」值），原串即一切筛选面的键（客户端 chip、工具参数），**翻译只发生在展示边缘**：client 持 6 个已知键的 zh/en 标签表，未知键剥 `^\d+-` 前缀后以原名示人（两种语言同名）——源目录出现新分类零改动可用；**② chip 从数据派生**——不设固定注册表（区别于姊妹插件 shipping 冻结快照的做法），按当前表实时计数排序，数字前缀天然保序，「未分类」桶仅在有缺类卡片时殿后；分类 chip 与状态 chip（全部/已装/可更新/含技能/团队）为**正交两维**，与 query 三维可叠；搜索 haystack 纳入分类的全部拼写（原串/zh 标签/en 标签，跨语言命中与姓名字段同律）；卡片带本地化分类 badge；census 增「分类 N」（非空去重值计数）；团队分组默认展开跟随 filteredActive 含分类维；**③ 工具面**——`workbuddy_experts` 增可选 `category` 参数（原串精确匹配、trim 后比对，无类专家永不命中），列表条目与 render 携 category（模型可见即可匹配），命中为空时给专属指引（不带 category 的调用即发现路径），canonical value 回显过滤键；**④ 零额外失效面**——plugin.json 的 stat 已在指纹清单内（#12/#5），分类变更天然触发重扫，无新缓存键 | §2 §6 `src/scanner.js` `client/client.js` `src/summon.js` |
